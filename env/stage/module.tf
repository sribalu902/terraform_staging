...
module "security_group_eks" {
  ...
  create_vpc = length(module.vpc.vpc_ids)

  bastion_sg_ids = module.bastion.bastion_sg_ids
  tags           = var.tags
}


############################################
# EKS NODEGROUP SERVICE-LINKED ROLE
...
module "eks" {
  ...
  # Ensure service-linked role exists first
  depends_on = [
  ...
  ############################################
  # SECURITY GROUPS (CUSTOM, MANAGED BY YOU)
  ############################################
  ...
  ############################################
  # NODE GROUPS (ON-DEMAND + SPOT SUPPORTED)
  ############################################
